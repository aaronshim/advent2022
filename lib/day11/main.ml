...
type monkey = Bigint.t list * operation * int * int * int * int [@@deriving sexp]
...
let parse_monkey lines : monkey =
  ...
  let items = List.nth_exn lines 1 |> Shared.extract_ints |> List.map ~f:Bigint.of_int in
...
let process_monkey (divide_by_3 : bool) (monkeys : monkey array) (i : int) : monkey array =
  ...
  let rec process_monkey' items num_inspected' =
    ...
    | item :: items' ->
      let worry_level =
        match op with
        | Plus n -> Bigint.( + ) (Bigint.of_int n) item
        | Times n -> Bigint.( * ) (Bigint.of_int n) item
        | PlusOld -> Bigint.( + ) item item
        | TimesOld -> Bigint.( * ) item item
      in
      let worry_level' =
        if divide_by_3 then Bigint.( / ) worry_level (Bigint.of_int 3) else worry_level
      in
      let worry_level_check =
        Bigint.( = ) (Bigint.( % ) worry_level' (Bigint.of_int test)) Bigint.zero
      in
...
module Part2 = struct
  ...
  let%expect_test "Solve small input" =
    ...
    [%expect
      {|
      (((8 12 63607185775640360 34) (Times 19) 23 2 3 54652)
       ((20 22 26 43 30 30) (Plus 6) 19 2 0 45344) (() TimesOld 13 1 3 454)
       (() (Plus 3) 17 0 1 54864)) |}]
  ;;
